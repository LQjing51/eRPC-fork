...
void send_req(AppContext *c, size_t msgbuf_idx) {
  ...
  if (c->fastrand_.next_u32() % 100 < FLAGS_get_req_percent) {
    ...
    *value_64 = (*key_32)*0x12345+0x10501;
  }

  *reinterpret_cast<wire_req_t *>(req_msgbuf.buf_) = req;

  c->client.window_[msgbuf_idx].req_key = (req.req_type == kAppGetFgType || req.req_type == kAppGetBgType)? req.get_req.key : req.put_req.key;
...
void app_cont_func(void *_context, void *_msgbuf_idx) {
  ...
  if (req->req_type == kAppGetFgType || req->req_type == kAppGetBgType) {
    c->client.get_latency.update(static_cast<size_t>(usec * 10.0));  // < 1us

    {
      ...
      if ((static_cast<uint64_t>(*key)*0x12345+0x10501) != *recvd_value) {
        fprintf(stderr,
                "KV Get Value mismatch. Req key = %u, recvd_value = %lu\n",
                *key, *recvd_value);
      }
    }
  } else {
    c->client.put_latency.update(static_cast<size_t>(usec * 10.0));
...
void client_print_stats(AppContext &c) {
  ...
  stats.lat_us_50 = c.client.get_latency.perc(0.50) / 10.0;
  stats.lat_us_90 = c.client.get_latency.perc(0.90) / 10.0;
  stats.lat_us_99 = c.client.get_latency.perc(0.99) / 10.0;
  ...
  printf(
      ...
      stats.lat_us_99, c.client.put_latency.perc(.5)/10.0, c.client.put_latency.perc(.9)/10.0, c.client.put_latency.perc(.99)/10.0);

  c.client.num_resps_tot = 0;
  c.client.get_latency.reset();
  c.client.put_latency.reset();
